...
- Next.js App Router의 서버 컴포넌트에서 포트폴리오 데이터를 요청마다 렌더링
- Route Handler(`/api/portfolio`)를 통한 데이터 제공
- Redux와 Redux-Saga로 비동기 데이터 흐름을 관리하고 서버 상태를 클라이언트에 전달
...
Server Component
  ...
  → /api/portfolio
  → preloadedState 생성
  → 클라이언트 Redux store hydration
```

포트폴리오의 실제 콘텐츠는 `src/data/portfolio.js`에서 관리합니다. API Route가 이 데이터를 반환하고, 서버에서 미리 구성한 Redux 상태를 각 섹션 컴포넌트에 전달합니다.
...
배포 환경에서 서버가 내부 API 주소를 자동으로 결정하지 못하는 경우 `NEXT_PUBLIC_SITE_URL`에 서비스의 전체 URL을 설정할 수 있습니다.
